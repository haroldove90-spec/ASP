...
import { SmartSupabaseModal } from './components/SmartSupabaseModal';
...
export default function App() {
  ...
  const checkSupabaseHealth = async () => {
    ...
    try {
      ...
      return { connected: false, latencyMs: 0, tables: { cotizaciones: 0, ordenes_trabajo: 0, instrumentos: 0, usuarios: 0, certificados_calibracion: 0 }, lastChecked: new Date().toLocaleTimeString('es-MX'), error: err?.message || String(err) };
    }
  };

  // Monitoreo Periódico en Segundo Plano (Heartbeat cada 25 segundos)
  useEffect(() => {
    let isMounted = true;
    checkSupabaseHealth();

    // Sondeo periódico automático cada 25 segundos sin bloquear navegación
    const heartbeatInterval = setInterval(() => {
      ...
      testSupabaseConnection().then((res) => {
        ...
        if (res.connected) {
          ...
          setSupabaseStatus('error');
          setSupabaseMessage(res.error || 'Desconexión de red con Supabase');
        }
      }).catch((err) => {
        if (!isMounted) return;
        setSupabaseStatus('error');
        setSupabaseMessage('Heartbeat error: ' + (err?.message || String(err)));
      });
    }, 25000); // 25s
  ...
  const handleApproveCertificate = async (cert: CertificadoCalibracion) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        serverSello = resData.sello_digital;
      }
    } catch {
      // client-side fallback signature
    }

    if (!serverSello) {
      serverSello = `NOM151:SELLO-CERT-${Date.now().toString(16).toUpperCase()}`;
    }
  ...
  return (
    <div className="flex h-screen w-full bg-slate-100 text-slate-800 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-3 self-stretch sm:self-auto">
            {/* Botón Inteligente Supabase con Semáforo 🟢 / 🔴 y Latencia */}
            <SmartSupabaseButton
              ...
              isVerifying={supabaseStatus === 'testing'}
            />

            {/* Sincronización Rápida Global */}
            <button
              type="button"
              onClick={handleGlobalSyncAll}
              disabled={isSyncingGlobal}
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 rounded-xl text-xs font-bold transition shadow-2xs cursor-pointer active:scale-98 disabled:opacity-50"
              title="Sincronizar todos los módulos (Agenda, Cotizaciones, Instrumentos, Personal, Bitácora) con Supabase"
            >
              <RefreshCw className={`w-3.5 h-3.5 text-[#85AA1C] ${isSyncingGlobal ? 'animate-spin' : ''}`} />
              <span className="hidden md:inline">{isSyncingGlobal ? "Sincronizando..." : "Sincronizar"}</span>
            </button>
      ...
      {/* MODAL INTELIGENTE Y CENTRO DE CONTROL SUPABASE */}
      <SmartSupabaseModal
        isOpen={isSupabaseModalOpen}
        onClose={() => setIsSupabaseModalOpen(false)}
        supabaseStatus={supabaseStatus}
        supabaseDiagnostics={supabaseDiagnostics}
        onCheckHealth={checkSupabaseHealth}
        onSyncAll={handleGlobalSyncAll}
        isSyncingGlobal={isSyncingGlobal}
        scheduledServicesCount={scheduledServices.length}
        quotesCount={generatedQuotes.length}
        instrumentsCount={instruments.length}
        usuariosCount={usuarios.length}
        auditLogs={auditLogs}
      />

      {/* SAVE TELEMETRY TOAST (NOTIFICACIÓN EMERGENTE TRAS CADA GUARDADO) */}
      <SaveTelemetryToast onOpenSupabaseModal={() => setIsSupabaseModalOpen(true)} />
